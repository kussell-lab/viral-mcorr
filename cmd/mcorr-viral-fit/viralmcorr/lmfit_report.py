...
def main():
    ...
    parser.add_argument("--phiS_init", type=float, help="set initial value for phiS in the fitting", default=0.00005)
    parser.add_argument("--f_init", type=float, help="set initial value for f in the fitting", default=1000)
    parser.add_argument("--thetaS_init", type=float, help="set initial value for thetaS in the fitting", default=0.00001)
    parser.add_argument("--phiS_max", type=float, help="set initial value for f in the fitting", default=1.0)
    ...
    title = opts.title
    f_init = opts.f_init
    thetaS_init = opts.thetaS_init
    phiS_init = opts.phiS_init
    phiS_max = opts.phiS_max
    ...
    # prefix = 'cluster8_cluster221_CORE_FIT_OUT'
    # fit_start = 3
    # fit_end = 300
    # quiet = False
    # use_geom_frag = False
    ...
    fitres = vary_fit(all.xvalues, all.yvalues, all.d_sample, r1_func, f_init, thetaS_init, phiS_init, phiS_max)
    # lmfitreport=fit_report(fitres)
    # print(lmfitreport)
    ...
    with open(lmfitfile, "w+") as csvfile:
        ...
        lmfit_writer.writerow(["ds", "thetaS", "f", "phiS", "thetaP", "phiP", "c",
                               "dp", "dc", "chisq", "red-chisq"])
        lmfit_writer.writerow([params["ds"], params["thetaS"], params["f"], params["phiS"],
                               params["thetaP"], params["phiP"],
                               params["c"], params["dp"], params["dc"], fitres.chisqr, fitres.redchi])
